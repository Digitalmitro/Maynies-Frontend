...
function DashboardLayout() {
  ...
  useEffect(() => {
    ...
    if (!role || !id) {
      setIsAuth(false);
      navigate("/login"); // agar auth nhi hai toh login pe bhej de
    } else {
      setIsAuth(true);
    }
  }, [navigate]);
  ...
  if (isAuth === null) {
    // loading screen dikha sakta hai tu
    return (
  ...
  return (
    <div className="flex min-h-screen bg-[#EDECEC] relative">
      ...
      {sidebarOpen && (
        <div 
          className="fixed inset-0 bg-black/70 bg-opacity-40 z-40 md:hidden" 
          onClick={() => setSidebarOpen(false)} 
        />
      )}
      
      <div className={`fixed top-0 left-0 h-full w-[75%] max-w-[280px] bg-white z-50 transform transition-transform duration-300 ease-in-out md:hidden ${
